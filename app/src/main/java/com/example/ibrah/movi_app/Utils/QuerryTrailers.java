...
public class QuerryTrailers {
     ...
     */
    private static List<Trailer> extractFeatureFromJson(String MovieJSON) {
        ...
        try {
            ...
            for (int i = 0; i < baseJsonResponseJSONArray.length(); i++) {
                ...
                String id = currentObjectInJasonArray.getString("id");
                String name = null;
                if (currentObjectInJasonArray.has("name")) {

                    name  = currentObjectInJasonArray.getString("name");

                }
                Log.i(LOG_TAG, " LINK OF THE IMAGE IS---------------- : " + i + ":  " + name);
                ///////////////
                String key = null;
                if (currentObjectInJasonArray.has("key")) {
                    // Extract the value for the key called "infoLink"
                    key = currentObjectInJasonArray.getString("key");
                }

                // Add the new {@link Earthquake} to the list of Books.
                Log.i(LOG_TAG, id + "" + key + "" + name);
